...
if (Meteor.isServer) {
  ...
  it('should not update note if additional updates', function() {
    expect(() => {
      const addition = 'additional update'
      Meteor.server.method_handlers['notes-update'].apply({
        userId: noteOne.userId,
      }, [
        noteOne._id,
  ...
  it('should return 0 notes for user that has none', function() {
    const res = Meteor.server.publish_handlers.notes.apply({ userId: 'fake'});
    const notes = res.fetch();
